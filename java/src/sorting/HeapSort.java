...
public class HeapSort {
	...
	private static <T> T[] heapify(T[] array, int rootIndex,
			Comparator<T> comparator) {
		return heapify(array, array.length, rootIndex, comparator);
	}

	private static <T> T[] heapify(T[] array, int length, int rootIndex,
			Comparator<T> comparator) {
		int indexToBeSwapped = rootIndex;

		if (leftChildIndex(rootIndex) < length) {
			if (comparator.compare(array[leftChildIndex(rootIndex)],
					array[indexToBeSwapped]) > 0) {
				indexToBeSwapped = leftChildIndex(rootIndex);
			}
		}

		if (rightChildIndex(rootIndex) < length) {
			if (comparator.compare(array[rightChildIndex(rootIndex)],
					array[indexToBeSwapped]) > 0) {
